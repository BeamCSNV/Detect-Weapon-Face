...
while True:
    ...
    for face in faces:
        ...
        if face_id_str not in known_face_encodings:
            ...
            # ค้นหาชื่อจาก known_face_encodings
            for known_id, known_name in zip(known_face_encodings, known_face_names):
                if face_id_str == known_id:
                    face_name = known_name
                    break

            # แสดงชื่อในภาพ
            cv2.putText(frame, face_name, (face.left(), face.top() - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
            ...
            if not os.path.exists(face_folder_path):
                ...
                # เพิ่มข้อมูลใบหน้าใหม่ลงในรายการที่รู้จัก
    ...
    if faces and not weapon_results:
        ...
        # แสดงผลลัพธ์ทาง画面
        cv2.imshow("ผลลัพธ์การตรวจจับ", weapon_and_face_image)
